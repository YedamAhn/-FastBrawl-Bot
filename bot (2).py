...
class ReviewAnonymousView(discord.ui.View):
    ...
    @discord.ui.button(label="Post with Username", style=discord.ButtonStyle.primary)
    ...
    @discord.ui.button(label="Post Anonymously", style=discord.ButtonStyle.secondary)
...
class ConfirmOrderView(discord.ui.View):
    ...
    @discord.ui.button(label="✅ Confirm & Create Ticket", style=discord.ButtonStyle.success, custom_id="confirm_order")
...
async def setup_ranked(interaction: discord.Interaction):
    await interaction.channel.send(view=RankedOrderView())
    await interaction.response.send_message("✅ Ranked order button posted!", ephemeral=True)
